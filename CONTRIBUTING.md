...
1. **No app-owned network calls, and no user data on the wire by default.** No cloud transcription, no cloud LLMs, no telemetry, no crash reporters, no analytics SDKs, no remote fonts. Exactly two system-mediated exceptions exist: iOS downloading Apple's on-device model assets (speech / Apple Intelligence), which never carries meeting content, and — only when the user turns on the off-by-default iCloud Backup toggle in Settings — iOS including meeting data in the device's own iCloud/computer backup. The app itself never opens a connection either way. If a feature needs a server, it doesn't belong in Minute.
