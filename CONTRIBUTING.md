...
5. **No new third-party dependencies** without prior discussion in an issue. The audited exceptions today are all on-device model runtimes for optional features — FluidAudio (speaker identification), WhisperKit (optional transcription), and MLX plus the Hugging Face packages that fetch and tokenize those models. Auditability, privacy-manifest coverage, licensing, model-host behavior, binary size, and removal strategy are part of any dependency review.
...
- **Content screens are reading surfaces, not stacked cards.** Meeting notes and entity pages are a `ScrollView` + `LazyVStack` using the shared `SectionHeading`, `BulletRow`, and `Layout` primitives in `Shared/Theme.swift`; separation comes from `Layout.sectionGap`, not from a card per section. Lists use `.listStyle(.plain)` with custom rows. Settings is deliberately a grouped `List` — a form should look like a form.
- **Glass belongs to the floating layer only** — toolbars, the New Meeting bar (`safeAreaBar`), status pills. Never put `.glassEffect` behind text the user is reading. Pair a `safeAreaBar` with `.scrollEdgeEffectStyle(.soft, …)` so content fades under it instead of colliding.
- Beware `.textCase(.uppercase)`: it rewrites the *accessibility label*, not just the rendering, and a child `.accessibilityLabel` cannot override it inside `.accessibilityElement(children: .combine)`. Use sentence case for anything VoiceOver or a UI test names.
