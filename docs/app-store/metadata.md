...
- Brain deletion is wired and covered by tests (`MinuteTests/KnowledgeDeletionTests.swift`): `MeetingStore.delete` calls `KnowledgeStore.reconcile`, which strips the deleted meeting from every fact's sources and removes the facts and entities left with no surviving source; `MeetingListView` runs the same reconcile at launch to catch what an interrupted delete, or a build predating this, left behind. Re-confirm on the release binary that deleting a meeting empties its entities, since the product copy above now claims it.
