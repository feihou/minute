...
- **Summaries** run on Apple Intelligence by default, which needs an Apple Intelligence–capable iPhone with it turned on (in the simulator, that means the host Mac has it enabled). Without such a device, pick a local model in Settings and summaries run through MLX instead
- **Brain requires Apple Intelligence and has no local-model alternative.** Extraction and synthesis both call Apple's on-device model directly, so choosing the local summary engine does not enable Brain — the tab explains this on devices that can't run it
