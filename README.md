...
| Delete means delete | Removing a meeting removes its saved audio file, its database row, and the Brain facts extracted from it — including any entity left with nothing else to say, since its name was learned from that meeting too. Orphaned audio and orphaned facts are both swept at launch |
